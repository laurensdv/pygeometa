...
from pygeometa.validation.harvested_metadata import ISODocument
    ...
    def is_valid(cls, xml):
        ...
        if metadata_type in ('dataset', 'series'):
            ...
            if not is_valid:
                # TODO: not sure if we need this one, keeping for backwards compatibility
                errors.insert(0,
                              ('{0} Validation Error'.format(xsd_name), None))
            ...
            is_valid, errors = cls._is_valid(xml, gmx_and_srv_xsd_filepath,
                                             xsd_name)
            if not is_valid:
                # TODO: not sure if we need this one, keeping for backwards compatibility
                errors.insert(0,
            ...
            is_valid = False
            errors = [(
                      'Metadata type not recognised "%s" - cannot choose an ISO19139 validator.' %
                      metadata_type, None)]
        if is_valid:
            return True, []
...
class FGDCSchema(XsdValidator):
    ...
    def is_valid(cls, xml):
        ...
        return cls._is_valid(xml, xsd_filepath,
                             'FGDC Schema (fgdc-std-001-1998.xsd)')
...
class SchematronValidator(BaseValidator):
    ...
    def extract_error_details(cls, failed_assert_element):
        ...
        return message, 'Error Message: "%s"  Error Location: "%s"  Error Assert: "%s"' % (
    ...
    def is_valid(self, xml):
        ...
        for name in self.profiles:
            ...
            if not is_valid:
                ...
                log.info(
                    'Validating against "%s" profile failed' % validator.title)
